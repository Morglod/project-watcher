...
var ProjectWatcher = /** @class */ (function () {
    function ProjectWatcher(rootPath, opts) {
        ...
        ['newDir', 'newFile'].forEach(function (event) {
            _this.watcher.on(event, function (path) {
                if (env_1.isDev)
                    console.log("[" + event + "] for '" + path + "'");
                var localPath = takeLocalPath(rootPath, path);
                // only empty entities
                var fileStat = fs.statSync(path);
                if (fileStat.isDirectory() && fs.readdirSync(path).length !== 0) {
                    if (env_1.isDev)
                        console.log("[" + event + "] for '" + path + "' fileStat.isDirectory() && fs.readdirSync(path).length !== 0");
                    return;
                }
                if (fileStat.isFile() && fileStat.size !== 0) {
                    if (env_1.isDev)
                        console.log("[" + event + "] for '" + path + "' fileStat.size !== 0; fileStat.size=" + fileStat.size);
                    return;
                }
                _this.paths.some(function (_a) {
                    var rule = _a.rule, opts = _a.opts;
                    ...
                    if (rule.match(localPath)) {
                        ...
                        if (event === 'newDir' && opts.newDirTemplate)
                            copyDirTemplate(path, opts.newDirTemplate, opts.replace, opts.replaceFileName);
                        if (event === 'newFile' && opts.newFileTemplate)
                            copyFileTemplate(path, opts.newFileTemplate, opts.replace);
                        if (opts.break)
                            return true;
                    }
                    return false;
...
function copyDirTemplate(dst, from, replacements, replaceFileName) {
    if (env_1.isDev)
        console.log("copyDirTemplate from '" + from + "' to '" + dst + "'");
...
function copyDir(dst, from, targetDirName, replacements, replaceFileName) {
    if (env_1.isDev)
        console.log("copyDir from '" + from + "' to '" + dst + "'");
    if (!fs.existsSync(dst))
        fs.mkdirSync(dst);
    var entries = fs.readdirSync(from);
    entries.forEach(function (x) {
        if (fs.statSync(path_1.join(from, x)).isDirectory())
            copyDir(path_1.join(dst, x), path_1.join(from, x), targetDirName, replacements, replaceFileName);
        else
            copyFile(path_1.join(dst, x), path_1.join(from, x), targetDirName, replacements, replaceFileName);
    });
}
exports.copyDir = copyDir;
function copyFileTemplate(dst, from, replacements) {
    if (env_1.isDev)
        console.log("copyFileTemplate from '" + from + "' to '" + dst + "'");
    var targetDirName = path_1.basename(dst);
    copyFile(dst, from, targetDirName, replacements);
}
exports.copyFileTemplate = copyFileTemplate;
function copyFile(dst, from, targetDirName, replacements, replaceFileName) {
    if (env_1.isDev)
        console.log("copyFile from '" + from + "' to '" + dst + "'");
    if (replaceFileName) {
