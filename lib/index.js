...
exports.REPLACE_FILE_NAME = '{{FILE_NAME}}';
/**
 * Core
 *
                ...
                _this.paths.some(function (_a) {
                    ...
                    if (rule.match(localPath)) {
                        ...
                        if (event === 'newDir' && opts.newDirTemplate)
                            copyDirTemplate(path, opts.newDirTemplate, opts.replace);
...
function copyDirTemplate(dst, from, replacements) {
    if (env_1.isDev)
...
function copyDir(dst, from, targetDirName, replacements) {
    if (env_1.isDev)
    ...
    entries.forEach(function (x) {
        if (fs.statSync(path_1.join(from, x)).isDirectory())
            copyDir(path_1.join(dst, x), path_1.join(from, x), targetDirName, replacements);
        else
            copyFile(path_1.join(dst, x), path_1.join(from, x), targetDirName, replacements);
    });
}
...
function copyFile(dst, from, targetDirName, replacements) {
    if (env_1.isDev)
    ...
    if (replacements) {
        ...
        for (var _i = 0, _a = Object.entries(replacements); _i < _a.length; _i++) {
            ...
            if (replaceTo === exports.REPLACE_FILE_NAME) {
