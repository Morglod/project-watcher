...
function copyDir(dst, from, replacements) {
    if (env_1.isDev)
    ...
    entries.forEach(function (x) {
        if (fs.statSync(path_1.join(from, x)).isDirectory())
            copyDir(path_1.join(dst, x), path_1.join(from, x), replacements);
        else
            copyFile(path_1.join(dst, x), path_1.join(from, x), replacements);
    });
}
...
function copyFile(dst, from, replacements) {
    if (env_1.isDev)
